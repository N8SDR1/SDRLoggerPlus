...
This document outlines the feasibility, requirements, and implementation steps for adding 32-bit Windows (win-x86/ia32) support to SDRLoggerPlus.
...
### Current State
SDRLoggerPlus currently builds and distributes artifacts for:
- **Windows**: x64 only
...
### 1. Electron Configuration (`src/SDRLoggerPlus.Desktop/electron-builder.yml`)
...
### 2. Electron Main Process (`src/SDRLoggerPlus.Desktop/main.js`)
...
### 3. Package.json Scripts (`src/SDRLoggerPlus.Desktop/package.json`)

**Add new scripts:**
```json
"copy:frontend:win-x86": "cp -r ../SDRLoggerPlus.Web/dist ../SDRLoggerPlus.Server/bin/Release/net10.0/win-x86/publish/wwwroot",
"build:backend:win-x86": "dotnet publish ../SDRLoggerPlus.Server -c Release --self-contained -r win-x86 -o ../SDRLoggerPlus.Server/bin/Release/net10.0/win-x86/publish && npm run copy:frontend:win-x86",
...
### 4. Native Hamlib Libraries (`src/SDRLoggerPlus.Server/runtimes/`)

**Create new directory:**
```
src/SDRLoggerPlus.Server/runtimes/win-x86/native/
```
...
Update `src/SDRLoggerPlus.Server/runtimes/README.md`:
...
5. Create installer: `SDRLoggerPlus-<version>-win-ia32.exe`
...
### Artifact Naming
- **64-bit**: `SDRLoggerPlus-<version>-win-x64.exe`
- **32-bit**: `SDRLoggerPlus-<version>-win-ia32.exe` (or `win-x86`)
...
Adding 32-bit Windows support to SDRLoggerPlus is **technically feasible and recommended**. The implementation effort is minimal (5-8 hours), all dependencies are compatible, and it extends the application's reach to users on legacy hardware. Given the amateur radio community's mix of modern and older equipment, providing 32-bit support aligns with the project's accessibility goals.
