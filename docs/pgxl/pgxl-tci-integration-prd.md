...
The existing PGXL integration in SDRLoggerPlus provides:
...
To integrate TCI radios with PGXL, SDRLoggerPlus will emulate a FlexRadio:
...
flowchart TB
    ...
    subgraph SDRLoggerPlus["SDRLoggerPlus Server"]
...
sequenceDiagram
    ...
    participant L4Y as SDRLoggerPlus<br/>Server
...
**Good news!** SDRLoggerPlus already has VITA-49 discovery packet broadcasting via the **SmartUnlink** feature. The `SmartUnlinkService` provides:
...
Create a new `FlexEmulatorService` in SDRLoggerPlus.Server:
...
1. **Connect TCI radio** to SDRLoggerPlus via WebSocket
2. **Enable FlexRadio Emulator** in SDRLoggerPlus settings
3. **Configure PGXL** to pair with emulated serial:
   - Open PGXL Settings in SDRLoggerPlus
...
flowchart TD
    subgraph Phase1["1️⃣ Startup"]
        A[SDRLoggerPlus starts FlexEmulator] --> B[Broadcasts VITA-49 discovery<br/>every 1 second]
    end
    ...
    subgraph Phase3["3️⃣ TCI Radio Connected"]
        G[TCI radio connects to SDRLoggerPlus] --> H[Frequency changes pushed<br/>to FlexEmulator]
