...
| File | Role |
|------|------|
| `src/SDRLoggerPlus.Web/src/plugins/MapPlugin.tsx` | Map rendering, marker icons, settings UI |
| `src/SDRLoggerPlus.Server/Hubs/LogHub.cs` | SignalR hub, QRZ lookup orchestration, MongoDB persistence |
| `src/SDRLoggerPlus.Contracts/Models/CallsignMapImage.cs` | MongoDB document model (`ImageUrl` is nullable `string?`) |
| `src/SDRLoggerPlus.Web/src/api/client.ts` | REST client + `CallsignMapImage` TypeScript interface (`imageUrl?` optional) |
| `src/SDRLoggerPlus.Web/src/store/appStore.ts` | Client-side state for `callsignMapImages[]` |
| `src/SDRLoggerPlus.Web/src/store/settingsStore.ts` | `showCallsignImages` and `maxCallsignImages` settings |
