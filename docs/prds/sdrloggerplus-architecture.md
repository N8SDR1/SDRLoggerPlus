...
flowchart TB
    ...
    subgraph ShackServer["SDRLoggerPlus Server (Shack PC / Raspberry Pi)"]
...
```
sdrloggerplus/
├── SDRLoggerPlus.sln                   # .NET solution file
├── package.json                 # Frontend workspace root
├── pnpm-workspace.yaml          # PNPM workspace config
├── docker-compose.yml           # Local dev / deployment
│
├── src/
│   ├── SDRLoggerPlus.Server/           # ASP.NET Core Backend
│   │   ├── Program.cs
│   │   ├── SDRLoggerPlus.Server.csproj
│   │   ├── appsettings.json
│   │   │
...
│   │
│   ├── SDRLoggerPlus.Contracts/        # Shared types between client & server
│   │   ├── SDRLoggerPlus.Contracts.csproj
...
│   │
│   └── SDRLoggerPlus.Web/              # React SPA (Frontend)
│       ├── src/
│       │   ├── App.tsx
│       │   ├── main.tsx
...
│   │   ├── SDRLoggerPlus.Plugin.Cluster/           # Backend (.NET)
│   │   │   ├── SDRLoggerPlus.Plugin.Cluster.csproj
...
│   │   ├── SDRLoggerPlus.Plugin.MapGlobe/          # Backend (.NET)
│   │   │   ├── SDRLoggerPlus.Plugin.MapGlobe.csproj
│   │   │   ├── MapGlobePlugin.cs
│   │   │   ├── Services/
...
│   │   ├── SDRLoggerPlus.Plugin.LogHistory/        # Backend (.NET)
│   │   │   ├── Services/
...
│       ├── SDRLoggerPlus.Plugin.LogEntry/          # Backend (.NET)
│       │   ├── Services/
│       │   │   ├── QrzLookupService.cs
...
2. **Contract Enforcement**: Shared types in `SDRLoggerPlus.Contracts` ensure API compatibility
...
flowchart TB
    subgraph Plugin["cluster/ (Full-Stack Plugin)"]
        subgraph Backend["SDRLoggerPlus.Plugin.Cluster/ (.NET)"]
        ...
        subgraph Shared["Via SDRLoggerPlus.Contracts"]
...
```csharp
// src/SDRLoggerPlus.Server/Plugins/IPlugin.cs
...
// Example: plugins/cluster/SDRLoggerPlus.Plugin.Cluster/ClusterPlugin.cs
...
```csharp
// src/SDRLoggerPlus.Server/Plugins/PluginLoader.cs
...
public static class PluginLoader
{
    public static void LoadPlugins(WebApplicationBuilder builder)
    {
        ...
        foreach (var dll in Directory.GetFiles(pluginDir, "SDRLoggerPlus.Plugin.*.dll"))
        {
...
```json
{
  "name": "@sdrloggerplus/plugin-cluster",
  "displayName": "DX Cluster",
  "version": "1.0.0",
  "description": "Real-time DX cluster spot integration",
  "author": "SDRLoggerPlus Team",
...
import { eventBus, EventMap } from '@sdrloggerplus/core';
...
```typescript
// src/SDRLoggerPlus.Web/src/hooks/useSignalR.ts
...
MongoDB is ideal for SDRLoggerPlus because:
...
```csharp
// src/SDRLoggerPlus.Contracts/Models/Qso.cs
...
```csharp
// src/SDRLoggerPlus.Server/Core/Database/Repositories/QsoRepository.cs
...
flowchart LR
    ...
    subgraph Shack["Shack Server"]
        Server[SDRLoggerPlus Server<br/>:3000]
    end
...
```mermaid
gantt
    title SDRLoggerPlus Development Phases
...
- [ ] Set up SDRLoggerPlus.Contracts for shared types
...
flowchart TB
    ...
    subgraph Docker["Docker Container"]
        Pull[docker pull sdrloggerplus/server]
        DockerRun[docker run -p 3000:3000<br/>-v ./data:/data<br/>--device /dev/ttyUSB0<br/>sdrloggerplus/server]
    end
...
services:
  sdrloggerplus:
    image: sdrloggerplus/server:latest
    build:
      context: .
      dockerfile: src/SDRLoggerPlus.Server/Dockerfile
    ports:
      - "5050:5050"              # HTTP
    ...
    environment:
      ...
      - MongoDB__DatabaseName=sdrloggerplus
  ...
  mongodb:
    ...
    environment:
      - MONGO_INITDB_DATABASE=sdrloggerplus
    restart: unless-stopped
...
```dockerfile
# src/SDRLoggerPlus.Server/Dockerfile
...
COPY src/SDRLoggerPlus.Server/*.csproj src/SDRLoggerPlus.Server/
COPY src/SDRLoggerPlus.Contracts/*.csproj src/SDRLoggerPlus.Contracts/
COPY plugins/*/SDRLoggerPlus.Plugin.*/*.csproj ./plugins/
RUN dotnet restore
...
RUN dotnet publish src/SDRLoggerPlus.Server -c Release -o /app/publish
