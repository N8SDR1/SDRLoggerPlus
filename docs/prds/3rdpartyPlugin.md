...
This document proposes an architecture to allow third-party developers to create plugins for SDRLoggerPlus. The goal is to enable external applications to interact with SDRLoggerPlus in a secure and documented way, allowing the community to extend the application's functionality.

Currently, SDRLoggerPlus has two types of "plugins":
...
The proposed architecture is based on a client-server model where third-party plugins are external clients that communicate with the SDRLoggerPlus server over the network.

- **Event Bus (Server -> Plugin):** Plugins will receive real-time events from SDRLoggerPlus by connecting to its **SignalR WebSocket endpoint**.
...
*(Note: A real diagram would be created here showing the relationship between SDRLoggerPlus Server, the WebSocket, the REST API, and an external plugin.)*
...
- A new section in the SDRLoggerPlus **Settings** page will be created for managing API keys.
...
- The SDRLoggerPlus server will have a new middleware that inspects incoming requests, validates the API key, and rejects any request that doesn't have a valid key (with a `401 Unauthorized` response).
...
- **Example Connection URL:** `ws://<sdrloggerplus_host>:<port>/loghub?apiKey=YOUR_API_KEY`
...
1.  **Develop the Plugin:** A developer writes their plugin in any language and on any platform they choose. The plugin's source code can be managed in its own Git repository, completely separate from SDRLoggerPlus's codebase. The developer's only dependencies are the SDRLoggerPlus API and WebSocket endpoints.

2.  **Generate an API Key:** The user runs SDRLoggerPlus, navigates to the `Settings -> API Keys` page, and generates a new API key for the plugin.

3.  **Configure the Plugin:** The user provides the plugin with two pieces of information:
    - The network address of the SDRLoggerPlus server (e.g., `localhost:5050`).
    - The generated API key.

4.  **Run the Plugin:** The user starts the plugin as a separate process (e.g., by running `python my_plugin.py` or double-clicking an executable). The plugin then connects to SDRLoggerPlus over the network.

This model means SDRLoggerPlus does **not** automatically discover or run plugins. The user is responsible for running their desired plugins alongside the main SDRLoggerPlus application.

### Comparison with a "Plugin Directory" Model

An alternative approach is a "plugin directory" model, where SDRLoggerPlus would monitor a special folder (e.g., `~/.sdrloggerplus/plugins`). Users would place plugin files in this folder, and SDRLoggerPlus would be responsible for loading and running them.

While this can offer a simpler user experience, it was not chosen for the initial design for the following reasons:

-   **Complexity:** SDRLoggerPlus would need complex logic to understand how to install dependencies and run plugins for various languages (e.g., `npm install`, `pip install`, `dotnet run`). This would be a significant maintenance burden.
...
-   **Flexibility:** The external model gives developers complete freedom. They can run the plugin on a different machine (e.g., a Raspberry Pi) and are not constrained by runtime environments or sandboxing that SDRLoggerPlus might impose.
...
SDRLOGGERPLUS_HOST = "localhost"
SDRLOGGERPLUS_PORT = 5050 # Adjust to the correct port
API_KEY = "your_generated_api_key"

API_BASE_URL = f"http://{SDRLOGGERPLUS_HOST}:{SDRLOGGERPLUS_PORT}/api"
WEBSOCKET_URL = f"ws://{SDRLOGGERPLUS_HOST}:{SDRLOGGERPLUS_PORT}/loghub?apiKey={API_KEY}"
...
async def callsign_lookup(callsign):
    """Calls the SDRLoggerPlus API to look up a callsign."""
    try:
...
async def event_listener():
    ...
    async with websockets.connect(WEBSOCKET_URL) as websocket:
        ...
        print("Connected to SDRLoggerPlus event stream...")
