...
This document outlines the integration of the 4O3A Antenna Genius 8x2 antenna switch into SDRLoggerPlus. The Antenna Genius is a network-enabled 8x2 antenna switch that allows two radios (A and B) to independently select from up to 8 configured antennas. This integration will provide real-time antenna status display and allow users to manually select antennas for each radio port directly from the SDRLoggerPlus interface.
...
Ham radio operators using the Antenna Genius currently need to use the separate Windows utility or manually configure their antenna selection. Integration with SDRLoggerPlus will provide:
...
Following the existing SDRLoggerPlus dark theme:
...
```
SDRLoggerPlus Server                    Antenna Genius
      |                                |
      |<---- UDP Discovery (1/sec) ----|
      |                                |
      |---- TCP Connect :9007 -------->|
...
```
Browser           SDRLoggerPlus Server           Antenna Genius
   |                    |                       |
   |-- SelectAntenna -->|                       |
   |    (port=1, ant=3) |                       |
   |                    |---- C6|port set 1 --->|
