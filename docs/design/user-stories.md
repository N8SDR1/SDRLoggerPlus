...
**As a** new user installing SDRLoggerPlus for the first time,
...
**As a** developer,
**I want** the `SDRLoggerPlus.Contracts` project to have no dependency on `MongoDB.Driver`,
**so that** shared models are provider-agnostic and can be used with any database backend.

**Acceptance Criteria:**

- [ ] `SDRLoggerPlus.Contracts.csproj` has no reference to `MongoDB.Bson` or `MongoDB.Driver`
...
- [ ] The SDRLoggerPlus logo/branding is visible in the wizard
