...
This document defines the UX flow for the redesigned setup wizard, updated Settings > Database section, and provider-aware status indicators. The goal is to make SDRLoggerPlus work immediately on first launch (via local LiteDB) while offering an optional cloud upgrade path.
...
(reuses existing SetupWizard form layout, slightly adapted)
  ...
  |                                                       |
  | Database Name                                         |
  | [SDRLoggerPlus________________________]                      |
...
|           SDRLoggerPlus/sdrloggerplus.db                            |
|                                                       |
| Size: 2.4 MB                                         |
| QSOs: 1,234                                          |
