...
This document provides a comparative analysis of POTA (Parks on the Air) logging features between SDRLoggerPlus and modern POTA logging tools (including Ham2K/potacat, HAMRS, and other popular solutions), identifies feature gaps, and proposes a prioritized implementation plan to make SDRLoggerPlus a competitive POTA logging solution.

## Background

SDRLoggerPlus currently has basic POTA support:
...
However, compared to dedicated POTA loggers, SDRLoggerPlus lacks specific activator/hunter workflows, POTA-specific ADIF fields, statistics, and offline capabilities.

## Comparative Analysis

### Feature Matrix: SDRLoggerPlus vs. Modern POTA Loggers

| Feature Category | SDRLoggerPlus (Current) | Ham2K/HAMRS/Modern Tools | Gap Priority |
|-----------------|------------------|-------------------------|--------------|
| **Spotting & Discovery** |
...
#### Critical Gaps
1. **No POTA-specific ADIF fields**: SDRLoggerPlus does not capture or export MY_SIG/MY_SIG_INFO (activator park) or SIG/SIG_INFO (hunter worked park)
...
**Goal**: Enable SDRLoggerPlus to be a viable activator logger with proper ADIF export
...
12. **POTA Integration**
    ...
    - Self-spotting from SDRLoggerPlus
...
- Maintain consistent design with existing SDRLoggerPlus UI
...
- SDRLoggerPlus Current Implementation:
  - `/src/SDRLoggerPlus.Web/src/plugins/POTAPlugin.tsx` - Spot display
  - `/src/SDRLoggerPlus.Server/Controllers/PotaController.cs` - POTA API proxy
  - `/src/SDRLoggerPlus.Server/Services/AdifService.cs` - ADIF import/export
  - `/src/SDRLoggerPlus.Contracts/Models/Qso.cs` - QSO data model
...
1. **Community Feedback**: Share this plan with SDRLoggerPlus users and POTA community for input
...
**Owner**: SDRLoggerPlus Development Team
