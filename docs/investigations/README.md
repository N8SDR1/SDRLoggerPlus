...
This directory contains the complete investigation into TCI (Transceiver Control Interface) multiple simultaneous connection support for SDRLoggerPlus.
...
- SDRLoggerPlus implementation analysis
...
- SDRLoggerPlus default: 50001 (incorrect)
- TCI standard: 40001 (correct)
- Location: `src/SDRLoggerPlus.Server/Services/TciRadioService.cs:27`
...
- SDRLoggerPlus built with port fix
- Another TCI-compatible application (JTDX, StationMaster, N1MM+)

### Test Procedure
1. Connect first application (SDRLoggerPlus)
...
2. If set to 40001: Works with other apps but SDRLoggerPlus may fail
3. If set to 50001: SDRLoggerPlus works but other apps may fail
4. **Temporary fix:** Set all apps and Thetis to same port

### After Fix is Released
1. Update SDRLoggerPlus to latest version
...
```
     ...
     ┌─────────┼─────────┐
     ↓         ↓         ↓
  SDRLoggerPlus    JTDX    StationMaster
...
2. **SDRLoggerPlus Compatibility:** Code is already multi-client compatible
...
- **Code:** `/src/SDRLoggerPlus.Server/Services/TciRadioService.cs`
...
- Review SDRLoggerPlus issue tracker for related discussions
