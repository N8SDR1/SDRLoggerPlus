...
User reported that only one application (SDRLoggerPlus or StationMaster) can connect to Thetis via TCI at a time. Investigation reveals this is likely a configuration issue, not a protocol limitation.
...
- SDRLoggerPlus uses default port **50001**
...
**File:** `src/SDRLoggerPlus.Server/Services/TciRadioService.cs`
...
**Files to Modify:**
- `src/SDRLoggerPlus.Contracts/Models/RadioConfigEntity.cs` - Add TciPort property
- `src/SDRLoggerPlus.Contracts/Models/Settings.cs` - Add default port setting
- `src/SDRLoggerPlus.Web/src/components/SettingsPanel.tsx` - Add port input field
- `src/SDRLoggerPlus.Server/Services/TciRadioService.cs` - Use configured port
...
**File:** `src/SDRLoggerPlus.Server.Tests/Tests/Services/TciRadioServiceTests.cs`
...
1. **Single Client Test**
   - Connect SDRLoggerPlus to Thetis
...
2. **Multi-Client Test**
   ...
   - Send command from SDRLoggerPlus → Thetis responds
   - Send command from JTDX → Thetis responds
   - Disconnect SDRLoggerPlus → JTDX remains connected
   ```

3. **Port Mismatch Test**
   - Configure SDRLoggerPlus for port 50001
   - Configure Thetis for port 40001
   - Verify clear error message
   - Update SDRLoggerPlus port to 40001
...
- If you've customized your Thetis TCI port to 50001, either:
  ...
  2. Manually configure SDRLoggerPlus to use port 50001 in settings

### New Feature: Port Configuration
You can now specify a custom TCI port in Radio Settings.

### Improvement: Multiple TCI Clients
SDRLoggerPlus now explicitly supports running alongside other TCI
...
- SDRLoggerPlus code doesn't prevent multiple connections
...
- Let SDRLoggerPlus act as TCI relay/proxy
...
┌───────────┬─────────────┬──────────────┐
↓           ↓             ↓              ↓
SDRLoggerPlus    JTDX     StationMaster    N1MM+
