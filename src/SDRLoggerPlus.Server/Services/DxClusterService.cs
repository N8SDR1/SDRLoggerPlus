...
public class DxClusterService : IDxClusterService, IHostedService, IDisposable
{
    ...
    /// </summary>
    internal Task IngestExternalSpotAsync(ParsedSpot spot, string sourceId, string sourceName)
        => OnSpotReceived(spot, sourceId, sourceName);
    ...
    private async Task SaveAndBroadcastSpotAsync(ParsedSpot parsedSpot, string source)
    {
        ...
        try
        {
            ...
            // Fall back to a cty.dat prefix lookup so the spotter-country
            // filter below works even without a QRZ result
            if (string.IsNullOrEmpty(spotterCountry))
            {
                (spotterCountry, spotterContinent) =
                    CtyService.GetCountryFromCallsign(parsedSpot.Spotter.Split('-')[0]);
            }

            // Global spotter-country filter — applies to ALL spot sources
            // (telnet clusters AND spothole), not just the spothole poller.
            var clusterSettings = (await scope.ServiceProvider
                .GetRequiredService<ISettingsService>().GetSettingsAsync()).Cluster;
            var countryFilter = clusterSettings.SpotholeSpotterCountry;
            // Only drop a spot when we positively know the spotter is in a
            // different country. Spots whose spotter country could not be
            // resolved are kept (dropping them silently starved the band-activity
            // heat map even within the configured country).
            if (!string.IsNullOrWhiteSpace(countryFilter) &&
                !string.IsNullOrEmpty(spotterCountry) &&
                !string.Equals(spotterCountry, countryFilter, StringComparison.OrdinalIgnoreCase))
            {
                _logger.LogDebug("Spot dropped by spotter-country filter: {Dx} by {Spotter} ({Country})",
                    parsedSpot.DxCall, parsedSpot.Spotter, spotterCountry ?? "unknown");
                return;
            }
        }
        catch (Exception ex)
        ...
        // spot sent before a client connects (e.g. spothole's startup backlog,
