...
public class HamlibController : ControllerBase
{
    ...
    /// <summary>
    /// Test TCP connection to a rotator controller and verify it responds, using the
    /// selected wire protocol (rotctld or microHAM ARCO). Reuses the same protocol
    /// strategies as the live RotatorService so the test matches real control.
    /// </summary>
    [HttpGet("test-rotator")]
    [ProducesResponseType(typeof(object), StatusCodes.Status200OK)]
    public async Task<ActionResult> TestRotatorConnection(
        [FromQuery] string host = "127.0.0.1",
        [FromQuery] int port = 4533,
        [FromQuery] string protocol = "rotctld")
    {
        var isArco = protocol == "arco_tcp";
        var label = isArco ? "ARCO controller" : "rotctld";
        try
        {
            ...
            if (raw is double azimuth)
            {
                ...
                return Ok(new { success = true, message = $"Connected! {label} at {azimuth:F0}\u00B0", azimuth });
            }

            return Ok(new { success = false, message = $"Connected to {host}:{port} but got no valid position from the {label}." });
        }
        catch (OperationCanceledException)
        {
            return Ok(new { success = false, message = $"Connection timeout. Check that the {label} is reachable at {host}:{port}." });
        }
        catch (Exception ex)
        {
