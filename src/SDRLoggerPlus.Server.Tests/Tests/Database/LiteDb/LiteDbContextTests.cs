...
public class LiteDbContextTests
{
    ...
    /// LiteDbContext uses GetDirectoryName(configPath) + "sdrloggerplus.db" as the DB file,
    ...
    public void DatabaseName_ReturnsFileName()
    {
        ...
        try
        {
            ctx.DatabaseName.Should().Be("sdrloggerplus.db");
        }
        finally
        {
            Cleanup(ctx, testDir);
    ...
    {
        var outerDir = Path.Combine(Path.GetTempPath(), $"sdrloggerplus_outer_{Guid.NewGuid():N}");
