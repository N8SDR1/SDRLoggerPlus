...
public class CtyServiceTests
{
    ...
    #region GetEntityFromCallsign (CQ zone)

    [Theory]
    [InlineData("W1AW", 5)]
    [InlineData("JA1ABC", 25)]
    [InlineData("VK2ABC", 30)]
    public void GetEntityFromCallsign_ReturnsCqZone(string call, int expectedZone)
    {
        var (country, continent, cqZone) = CtyService.GetEntityFromCallsign(call);
        country.Should().NotBeNull();
        continent.Should().NotBeNull();
        cqZone.Should().Be(expectedZone);
    }

    [Fact]
    public void GetEntityFromCallsign_Empty_ReturnsNulls()
    {
        CtyService.GetEntityFromCallsign("").Should().Be(((string?)null, (string?)null, (int?)null));
    }
