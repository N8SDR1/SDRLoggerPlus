...
describe('AboutDialog', () => {
  ...
  it('renders when isOpen is true', () => {
    ...
    expect(screen.getByText('SDRLoggerPlus')).toBeInTheDocument();
  });

  it('shows the banner graphic', () => {
    render(<AboutDialog isOpen={true} onClose={vi.fn()} />);
    const logo = screen.getByAltText('SDRLoggerPlus');
    expect(logo).toBeInTheDocument();
    expect(logo).toHaveAttribute('src', './sdrloggerplus-banner.png');
  });
  ...
  it('does not call onClose when dialog content is clicked', () => {
    const onClose = vi.fn();
    render(<AboutDialog isOpen={true} onClose={onClose} />);
    const dialogContent = screen.getByText('SDRLoggerPlus').closest('.glass-panel') as HTMLElement;
