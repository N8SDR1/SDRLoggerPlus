...
import { signalRService, type HamlibRigConfigDto, type SignalRConnectionState } from '../api/signalr';
    ...
    signalRService.setOnConnectedCallback(async () => {
      ...
      try {
        ...
        await Promise.all([
          ...
          useLayoutStore.getState().loadFromBackend(),
        ]);
      ...
      try {
        ...
        signalRService.setHandlers({
          ...
          onSpotReceived: (evt) => {
            ...
            // Hot List TTS announcement (per-callsign cooldown)
            if (evt.isHot) {
              const hotList = useSettingsStore.getState().settings.hotList;
              if (hotList.enabled && hotList.ttsEnabled &&
                  shouldAnnounce(evt.dxCall, hotList.ttsCooldownMinutes)) {
                const freqMhz = evt.frequency / 1000;
...
export function useSignalR() {
  ...
  const startRadioDiscovery = useCallback(async (type: 'Tci') => {
    await signalRService.startRadioDiscovery(type);
  }, []);

  const stopRadioDiscovery = useCallback(async (type: 'Tci') => {
