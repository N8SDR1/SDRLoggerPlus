...
export function GridTrackerPlugin() {
  ...
  // 'chart' = the original equirectangular grid chart; 'map' = the same grids painted on a
  // real slippy basemap (GridTracker-app look). Chart stays the default — it's kept intact.
  // View + basemap persist across sessions so the panel reopens where you left it.
  const [view, setView] = useState<'chart' | 'map'>(() => (localStorage.getItem('gt.view') === 'map' ? 'map' : 'chart'));
  const [basemap, setBasemap] = useState<TileLayerKey>(() => {
    const v = localStorage.getItem('gt.basemap');
    return v && v in TILE_LAYERS ? (v as TileLayerKey) : 'dark';
  });
  useEffect(() => { localStorage.setItem('gt.view', view); }, [view]);
  useEffect(() => { localStorage.setItem('gt.basemap', basemap); }, [basemap]);
  ...
  // Same live grids, but carrying the callsign(s) heard in each — for the Map view tooltips.
  const liveInfo = useMemo(() => {
    const m = new Map<string, { needed: boolean; calls: string[] }>();
    for (const d of decodes) {
      if (!d.grid) continue;
      if (fdd) {
        if (liveBand && d.band !== liveBand) continue;
      ...
      const g = d.grid.slice(0, 4).toUpperCase();
      const cur = m.get(g) ?? { needed: false, calls: [] };
      cur.needed = cur.needed || !worked.has(g);
      if (d.callsign && !cur.calls.includes(d.callsign)) cur.calls.push(d.callsign);
      m.set(g, cur);
    }
    return m;
  }, [decodes, worked, fdd, liveBand, cqOnly, neededOnly]);
    ...
    <GlassPanel title="Grid Tracker" icon={<Grid3x3 className="w-5 h-5" />}
      ...
      <div className="h-full flex flex-col">
        ...
        <div className="px-3 py-2 border-b border-glass-100 flex items-center gap-2 flex-wrap text-xs">
          {/* Chart ⇄ Map view toggle — Chart is the original grid chart, Map paints the same
              grids on a real basemap. */}
          <div className="flex rounded border border-dark-500 overflow-hidden">
            <button onClick={() => setView('chart')} title="Grid chart view"
              className={`px-2 py-1 flex items-center gap-1 ${view === 'chart' ? 'bg-cyan-500/20 text-cyan-300' : 'bg-dark-700 text-dark-300 hover:bg-dark-600'}`}>
              <LayoutGrid className="w-3.5 h-3.5" /> Chart
            </button>
            <button onClick={() => setView('map')} title="Map view — grids on a real basemap"
              className={`px-2 py-1 flex items-center gap-1 border-l border-dark-500 ${view === 'map' ? 'bg-cyan-500/20 text-cyan-300' : 'bg-dark-700 text-dark-300 hover:bg-dark-600'}`}>
              <MapIcon className="w-3.5 h-3.5" /> Map
            </button>
          </div>
          {view === 'map' && (
            <select value={basemap} onChange={(e) => setBasemap(e.target.value as TileLayerKey)}
              className="glass-input py-1 px-2 text-xs" title="Basemap">
        ...
        <div className="relative flex-1 min-h-0" style={{ background: '#0a1119' }}>
          {isLoading && <div className="absolute inset-0 z-[500] flex items-center justify-center text-dark-400 text-sm pointer-events-none">Loading grids…</div>}
          {view === 'chart' ? (
            <>
            ...
            <GridTrackerMapView worked={worked} liveInfo={liveInfo} myGrid={myGrid} basemap={basemap} />
          )}
        </div>
