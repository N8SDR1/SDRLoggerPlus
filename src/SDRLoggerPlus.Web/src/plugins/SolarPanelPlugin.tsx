...
import { api, type SpaceWeatherData } from '../api/client';
...
// K-Index geomagnetic condition label
function getKIndexLabel(kIndex: number): string {
  if (kIndex <= 2) return 'Quiet';
  if (kIndex === 3) return 'Unsettled';
  if (kIndex === 4) return 'Active';
  if (kIndex <= 6) return 'Storm';
  return 'Severe Storm';
}
...
function SolarIndicesView() {
  const [indices, setIndices] = useState<SpaceWeatherData | null>(null);
  const [error, setError] = useState(false);

  // Fetch live SFI / K-index / SSN from the backend (NOAA SWPC, HamQSL fallback).
  // The backend caches for 15 minutes, so polling at that cadence is sufficient.
  useEffect(() => {
    let cancelled = false;
    ...
    fetchIndices();
    const interval = setInterval(fetchIndices, 15 * 60 * 1000);
  ...
  if (!indices) {
    return (
      <div className="flex flex-col items-center justify-center h-full gap-3 text-gray-500">
        {error ? (
          <span className="text-sm text-center">Unable to load solar indices.</span>
        ) : (
    ...
    <div className="flex flex-col gap-6 h-full justify-center">
      ...
      <div className="glass-panel p-4">
        <div className="flex items-center justify-between">
          ...
          <span className="text-3xl font-mono font-bold text-accent-primary">{indices.solarFluxIndex}</span>
        </div>
        <div className="text-xs text-gray-500 mt-1">10.7 cm radio flux — higher favors HF</div>
      </div>
      ...
      <div className="glass-panel p-4">
        ...
        <div className="text-xs mt-1" style={{ color: getKIndexColor(indices.kIndex) }}>
          {getKIndexLabel(indices.kIndex)} — lower is better
        </div>
      </div>
      ...
      <div className="glass-panel p-4">
        <div className="flex items-center justify-between">
          ...
          <span className="text-3xl font-mono font-bold text-accent-primary">{indices.sunspotNumber}</span>
        </div>
        <div className="text-xs text-gray-500 mt-1">Daily sunspot count</div>
      </div>

      <div className="text-xs text-gray-500 text-center">
        Updated: {new Date(indices.timestamp).toLocaleString()}
      </div>
    </div>
  );
}
