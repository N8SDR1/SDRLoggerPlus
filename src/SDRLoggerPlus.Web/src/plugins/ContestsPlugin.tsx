...
// Format date/time for contests. Contest schedules are conventionally given in
// UTC (Z), so UTC is primary; the operator's local time follows in parentheses.
const formatDateTime = (dateStr: string): string => {
  const date = new Date(dateStr);
  const now = new Date();

  // Day label is based on the UTC calendar date (matches the UTC time shown).
  const todayUtc = Date.UTC(now.getUTCFullYear(), now.getUTCMonth(), now.getUTCDate());
  const tomorrowUtc = todayUtc + 24 * 60 * 60 * 1000;
  const contestUtc = Date.UTC(date.getUTCFullYear(), date.getUTCMonth(), date.getUTCDate());

  let dayLabel: string;
  if (contestUtc === todayUtc) {
    dayLabel = 'Today';
  } else if (contestUtc === tomorrowUtc) {
    dayLabel = 'Tomorrow';
  } else {
    dayLabel = date.toLocaleDateString('en-US', { month: 'short', day: 'numeric', timeZone: 'UTC' });
  }

  const pad = (n: number) => n.toString().padStart(2, '0');
  const utcTime = `${pad(date.getUTCHours())}${pad(date.getUTCMinutes())}Z`;
  const localTime = date.toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit' });

  return `${dayLabel} ${utcTime} (${localTime})`;
};
